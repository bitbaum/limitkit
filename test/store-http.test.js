...
test('clientIp: trustedProxies moves the hop, and 0 believes nothing', () => {
  ...
  assert.equal(clientIp(h({ 'x-forwarded-for': '203.0.113.7' }), { trustedProxies: 3 }), '203.0.113.7');
});
...
test('clientIp: absence degrades to a shared bucket, never a throw', () => {
  ...
  assert.equal(clientIp(h({ 'x-forwarded-for': ' , , ' })), 'unknown', 'separators alone are not an identity');
});
